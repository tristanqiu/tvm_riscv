...
def assert_partitioned_function(orig_mod, rvx_mod, expected_ops_unchanged=True):
    """
    ...
    """
    attrs = [
        rvx_mod[var.name_hint].attrs
        for var in rvx_mod.get_global_vars()
        if rvx_mod[var.name_hint].attrs
    ]
    ...
    assert any(compilers), "Module does not contain function for rvx target."
